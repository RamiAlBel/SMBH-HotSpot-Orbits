...
files = sorted(glob('/scratch/ralbe/meniar_and_django/smbh_hotspots_repository/data/raw/dataset_i0/*.dat'))
for file in tqdm.tqdm(files, total=len(files)):
    try:
        with open(file, 'r') as f:
            lines = f.readlines()
        
        # Line 2 has format: "# First row Period = X [M] ; Y [min]"  
        # Line 3 has two numbers: period_M period_min
        period_line = lines[2].strip().split()
        period_min = float(period_line[1])
        
        # Extract parameters from filename
        # Format: lc_rXX_KXXX_aXX_iX.dat
        parts = file.split('/')[-1].split('.')[0].split('_')
        r = float(parts[1][1:]) / 10
        ...
        for line in data_lines:
            ...
            try:
                # DPA is in column index 12 (13th column, -2 from end)
                dpa_val = float(cols[12])
...
# Save to CSV
output_path = '../../data/processed/dpa_dataset_i0.csv'
