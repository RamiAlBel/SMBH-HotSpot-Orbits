...
def main():
    ...
    for seed, outputs in per_seed_outputs.items():
        # Require at least spin and incl to exist; theta/z optional
        ...
        if "theta" in outputs:
            theta_true = outputs["theta"]["y_true"]
            theta_pred = outputs["theta"]["y_pred"]
            data.update({
        ...
        if "z" in outputs:
            z_true = outputs["z"]["y_true"]
            z_pred = outputs["z"]["y_pred"]
            data.update({
                "z_true": z_true,
                "z_pred": z_pred,
                "z_error": z_true - z_pred,
            })
