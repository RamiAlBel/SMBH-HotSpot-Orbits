...
def run_mc_checkpoint_mode(config, df, results_dir, exp_name, device):
    """Train one model per orbit-coverage level; save checkpoints for MC analysis.

    Invoked with:  python train.py [config.yaml] --mc

    Produces for each num_samples in sweep.num_samples_range and each target:
      results/checkpoints/<exp_name>/<n>samp/<target>/model_seed42.pth
      results/metrics/<exp_name>/<n>samp/<target>_test_details_seed42.csv
    """
    from src.training.data_loader import prepare_dataloaders as prep_dl
    MC_SEED = 42
    print("\n*** MC checkpoint mode: one model per orbit-coverage level (seed=42, no WandB) ***\n")

    for num_samples in config['sweep']['num_samples_range']:
        percent = num_samples * 10
        print(f"\n{'='*60}")
        print(f"Coverage: {percent}%  ({num_samples} DPA samples)")
        print(f"{'='*60}")
        ...
        for target_config in config['targets']:
            ...
            features, targets = build_features_partial_orbit(
                ...
                convert_to_radians, MC_SEED
            )

            train_loader, val_loader, test_loader, scaler_X, scaler_y, (_, _, test_idx) = prep_dl(
                features, targets,
            ...
            test_metrics, test_targets_arr, test_preds_arr = evaluate_model(
                model, test_loader, scaler_y, device
            )
            print(f"    MAE={test_metrics['mae']:.4f}  R²={test_metrics['r2']:.4f}  σ={test_metrics['error_std']:.4f}")

            ckpt_dir = results_dir / "checkpoints" / exp_name / f"{num_samples}samp" / target_name
            ckpt_dir.mkdir(parents=True, exist_ok=True)
            trainer.save_checkpoint(
                str(ckpt_dir / f"model_seed{MC_SEED}.pth"),
            ...
            metrics_dir = results_dir / "metrics" / exp_name / f"{num_samples}samp"
            metrics_dir.mkdir(parents=True, exist_ok=True)
            details_df = pd.DataFrame({
                "sample_idx": list(range(len(test_targets_arr))),
                "dataset_idx": list(test_idx),
                f"{target_name}_true": test_targets_arr,
                f"{target_name}_pred": test_preds_arr,
                f"{target_name}_error": test_targets_arr - test_preds_arr,
            })
            details_df.to_csv(
                str(metrics_dir / f"{target_name}_test_details_seed{MC_SEED}.csv"),
                index=False
            )

    print("\nMC checkpoint mode complete!")


def main():
    mc_mode = '--mc' in sys.argv
    if mc_mode:
        sys.argv = [a for a in sys.argv if a != '--mc']
    ...
    if mc_mode:
        run_mc_checkpoint_mode(config, df, results_dir, exp_name, device)
        return
